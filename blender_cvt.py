...
def convert_pkl_to_asset_list(
    ...
    if not pkl_list:
        LOGGER.warning("No PKL files found under %s", pkl_dir)
        return
    ...
    for pkl_file in pkl_list:
        ...
        if wTc.ndim != 3 or wTc.shape[-2:] != (4, 4):
            raise ValueError(
                f"Camera extrinsics must be (T,4,4); got shape {wTc.shape} for {index}."
            )
        if wTc.shape[0] != frame_count:
            raise ValueError(
                f"Extrinsic sequence length {wTc.shape[0]} does not match "
                f"pose length {frame_count} for {index}."
            )
        ...
        if gt_wTc_meta is None or len(gt_wTc_meta) == 0:
            LOGGER.warning(
                "Ground-truth wTc missing for sequence %s; skipping alignment.",
                seq_id,
            )
        else:
            try:
                ...
                LOGGER.error(
                    "Failed to align sequence %s due to error: %s. Falling back to identity.",
                    seq_id,
                    exc,
                )
        ...
        print("wTo_mats", wTo_mats.shape, obj_verts_exp.shape)
        ...
        for t in range(frame_count):
            ...
            if contact.size and contact.shape[-1] >= 2:
                if contact[t, 0] > contact_threshold:
                    left_color = [1.0, 0.3, 0.3, 1.0]
                if contact[t, 1] > contact_threshold:
                    right_color = [0.3, 0.3, 1.0, 1.0]
                if (contact[t] > contact_threshold).any():
                    object_color = [1.0, 0.8, 0.4, 1.0]
    ...
    if camera_mats:
        alloc_camera = compute_allocentric_camera(camera_mats)
